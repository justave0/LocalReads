...
public class SignOnAuthorActivity2 extends AppCompatActivity {
    ...
    public void getLatLng(Place place){
        String placeHTTPS = "https://maps.googleapis.com/maps/api/place/details/json?placeid="+place.getId()+"&key="+ getString(R.string.google_maps_api_key);
        AsyncHttpClient client = new AsyncHttpClient();
        client.get(placeHTTPS, new JsonHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Headers headers, JSON json) {
                Log.d(TAG,  "onSuccess");
                JSONObject jsonObject = json.jsonObject;
                try{
                    JSONObject results = (JSONObject) jsonObject.getJSONObject("result").getJSONObject("geometry").getJSONObject("location");
